                            ...
                            .onChange(of: gameViewModel.game.gameState) { gameState in
                                if gameState == .end {
                                    ...
                                    if gameViewModel.game.gameType == .rank {
                                        playerViewModel.updateGameDataForRank(isWin: gameViewModel.game.winner?.email == playerViewModel.currentPlayer.email) { result in
                                            switch result {
                                            case .success():
                                                appState = .endGame
                                        ...
                                        playerViewModel.updateGameDataForCasual(isWin: gameViewModel.game.winner?.email == playerViewModel.currentPlayer.email) { result in
                                            switch result {
                                            case .success():
                                                appState = .endGame
                                            case .failure(let error):
